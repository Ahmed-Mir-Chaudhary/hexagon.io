...
export function CaseStudyCard({ title, description, industry, image, slug, className }: CaseStudyCardProps) {
  return (
    <motion.div
      whileHover={{ y: -10 }}
      className={cn(
        "group relative overflow-hidden rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all duration-500",
        className
      )}
    >
      <div className="aspect-video overflow-hidden relative">
        ...
        <div className="absolute top-4 left-4">
          <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider bg-white/90 dark:bg-slate-900/90 text-brand-600 rounded-full backdrop-blur-sm">
            {industry}
          </span>
        </div>
      </div>
      <div className="p-8">
        ...
        <Link
          ...
          className="flex items-center justify-between w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 group-hover:bg-brand-600 group-hover:text-white transition-all duration-300"
        >
