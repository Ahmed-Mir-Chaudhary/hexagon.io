    ...
    >
      ...
      <div className="mb-8">
        ...
        <div className="flex items-baseline gap-1 mb-4">
          ...
          {price !== 'Custom' && <span className="text-slate-600">/mo</span>}
        </div>
        <p className={cn("text-sm leading-relaxed", popular ? "text-slate-400" : "text-slate-700")}>
          {description}
        </p>
      </div>
      <ul className="space-y-4 mb-8">
        {features.map((feature) => (
          <li key={feature} className="flex items-start gap-3 text-sm">
            ...
            <span className={popular ? "text-slate-300" : "text-slate-700"}>{feature}</span>
          </li>
        ))}
      </ul>
      <button
